...
define(['./lib/astro', './lib/streams'], function (astro, streams) {
  ...
  const OUT_EL = document.querySelector('#output')
  ...
  */
  ...
  function main () {
    let i, j, nday, nst, day1, mon1, year1, day2, mon2, year2,
      ...
      height, jd, jd1, jd2, gst, t, tt, tb, te, t24,
    ...
    if (jd1 > jd2) {
      console.error('Start date is later than end date. Please correct.')
      return
    }

    // TODO: get height here
    height = 100
    output(`Height of aiming point: ${height} km\n`);

    // TODO: get aiming point lat coords
    [deg, min, sec] = [52, 12, 47]
    output(`Geographical latitude of aiming point:  ${deg}° ${min}' ${sec}"`)
    if (deg >= 0) sign = +1
    else sign = -1
    phir = PI / 180 * (deg + sign * min / 60 + sign * sec / 3600);

    // TODO: get aiming point long coords
    [deg, min, sec] = [3, 34, 7]
    ...
    // TODO: get coords of the stations
    const coords = [
      { lat: [53, 11, 53], lon: [2, 44, 18] },
      { lat: [51, 18, 33], lon: [2, 24, 38] }
    ]
    coords.forEach((coords, i) => {
      [deg, min, sec] = coords.lat
      output(`Geographical latitude of station ${i + 1}:     ${min}° ${deg}' ${sec}"`)

      if (deg >= 0) sign = +1
      else sign = -1
      phi[i] = (PI / 180) * (deg + sign * min / 60 + sign * sec / 3600);

      [deg, min, sec] = coords.lon
      output(`Geographical longitude of station ${i + 1}:    ${min}° ${deg}' ${sec}"`)
      if (deg >= 0.0) sign = +1.0
      else sign = -1.0
    ...
    coords.forEach((coord, i) => {
      ...
      output(`\nAzimuth for station ${i + 1}:\t\t<b>${(az[i] / PS).toFixed(1)}°</b>`)
      output(`Elevation for station ${i + 1}:\t<b>${(elv[i] / PS).toFixed(1)}°</b>`)
    })
    ...
    output(`Julian Day ${jd1.toFixed(1)} corresponds to date <b>${year1}-${mon1}-${day1}</b>`)
